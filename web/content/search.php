...
if (strlen($search) >= 3) {
    ...
    if (valid_mac($search)) {
        ...
        if ($k == $bosskey)
            $sql = 'SELECT hex(nets.hash) as hash, nets.bssid AS bssid, nets.ssid AS ssid, nets.pass AS pass, nets.hits, nets.ts, nets.n_state AS n_state
FROM nets
WHERE bssid = ?
ORDER BY net_id DESC';
        else
            $sql = 'SELECT hex(nets.hash) as hash, nets.bssid AS bssid, nets.ssid AS ssid, IF(n.u_id IS NULL, IF(nets.pass IS NULL,NULL, \'Found\'), nets.pass) AS pass, nets.hits, nets.ts, nets.n_state AS n_state
