...
function put_work($mysql, $candidates) {
    ...
    $stmt = $mysql->stmt_init();
    $stmt->prepare("SELECT pass FROM (SELECT DISTINCT bssid, pass FROM nets WHERE n_state=1 AND (algo IS NULL OR algo = '') GROUP BY bssid, pass) t GROUP BY pass ORDER BY count(pass) DESC");
    $stmt->execute();
